...
By default, we look for rubies in `$XDG_DATA_HOME/rv/rubies`, `~/.local/share/rv/rubies`, `~/.rubies`, `/opt/rubies`, `/usr/local/rubies`.

By default, we install rubies into `~/.local/share/rv/rubies`.
